...
| *BUILDER_CONTEXT_DOCKER_OPTS* | `host=tcp://docker:2376,ca=/certs/client/ca.pem,cert=/certs/client/cert.pem,key=/certs/client/key.pem` | Options passed to `--docker` parameter from `docker context create` command, used by multiarch building when TLS is enabled at Docker daemon (DinD service). If you need to work with Docker daemon with TLS disabled (`2375` port), set `BUILDER_CONTEXT_DOCKER_OPTS="host=tcp://docker:2375"`. |
